...
def init_database():
    ...
    cursor.execute('''
    CREATE INDEX IF NOT EXISTS idx_timestamp ON occupancy_events(timestamp)
    ''')
    ...
    if not db_exists:
        print(f"{get_timestamp()} Database created at {DB_FILE}")
    else:
        print(f"{get_timestamp()} Connected to existing database at {DB_FILE}")

# Store occupancy event in database
def store_occupancy_event(occupancy_state, raw_data=None):
    """
    Store an occupancy event in the database.
    
    Args:
        occupancy_state (bool): True if presence detected, False otherwise
        raw_data (str, optional): Raw JSON data from the MQTT message
    """
    try:
        # Connect to the database
        conn = sqlite3.connect(DB_FILE)
        ...
        # Insert the event
        cursor.execute(
            "INSERT INTO occupancy_events (timestamp, occupancy, raw_data) VALUES (?, ?, ?)",
            (timestamp, occupancy_state, raw_data)
        )
        ...
        print(f"{get_timestamp()} Error storing event in database: {e}")
...
def on_message(client, userdata, msg, properties=None):
    ...
    try:
        ...
        if 'occupancy' in payload:
            ...
            # Only output and store when the state changes
            if current_state != last_occupancy_state:
                ...
                store_occupancy_event(current_state, raw_data)
